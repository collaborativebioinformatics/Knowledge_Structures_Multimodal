...
def main():
    ...
    if client_name == "site-1":
        ...
        update_model = clients.memphis.client.update_model()

    elif client_name == "site-2":
        print(f"--- {client_name}: San Diego ---")
        update_model = clients.san-diego.client.update_model()
    ...
    while flare.is_running():
        ...
        print(f"site = {client_name}, current_round={global_model.current_round}")
        new_params, steps = update_model(client_name, DATASET_PATH, model, device, summary_writer, testCDloader, testRNAloader, global_model)
